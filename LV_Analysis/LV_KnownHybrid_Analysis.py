...
for size in sizes:
    for replicate in range(replicates):
        ...
        while complete == False:
            model = hybridODE(p,(2,2,[size])).to(device)
            attempts += 1
        ...
        torch.save(model,f'Experiments/LV_KnownParamHybrid_Models/LV_KnownParamHybrid_{size}_{replicate}.pt')

df = pd.DataFrame({"Train Loss":Train, "Test Loss": Test, "Replicate": Replicates, "Size":Size_Record, "Attempts":Attempts})

df.to_csv("Experiments/LV_KnownParamHybrid_Results.csv",index=False)

"""
model = hybridODE(p,(2,2,[5])).to(device)
...
plt.savefig('LV_testing_KnownHybrid.png')
