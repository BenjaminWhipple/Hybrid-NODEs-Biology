...
plt.title('RMSE: Lotka-Volterra',fontsize=22)
...
plt.savefig("MeanRMSE_Plots_LV.png")
...
for i in unique_dims:
	...
	hybrid_err_bars.append(1.96*statistics.stdev(list(temp_hybrid['Mechanistic Error']))/np.sqrt(n_obs_hybrid))
	...
	node_err_bars.append(1.96*statistics.stdev(list(temp_node['Mechanistic Error']))/np.sqrt(n_obs_node))
	print()

plt.figure(figsize=(8,6))

plt.title('Mechanistic RMSE: Lotka-Volterra',fontsize=22)
#plt.plot(unique_dims, node_mech_err_means,label="Pure NODE")
#plt.plot(unique_dims, hybrid_mech_err_means,label="Hybrid NODE")
plt.errorbar(unique_dims, node_mech_err_means, yerr=node_err_bars,label="Pure NODE")
plt.errorbar(unique_dims, hybrid_mech_err_means,yerr=hybrid_err_bars,label="Hybrid NODE")
plt.xlabel('Network Size',fontsize=16)
plt.ylabel('Mean Mechanistic RMSE',fontsize=16)
plt.legend(fontsize=16)
