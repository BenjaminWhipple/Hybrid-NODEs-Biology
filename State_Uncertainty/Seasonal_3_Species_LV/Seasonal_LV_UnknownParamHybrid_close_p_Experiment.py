...
for size in sizes:
    for replicate in range(replicates):
        ...
        while complete == False:
            ...
            attempts += 1
            model = unknownhybridODE(p,(4,1,[size,size])).to(device)
        ...
        torch.save(model,f'Experiments/Seasonal_LV_UnknownParamHybrid_Close_Params/Seasonal_LV_UnknownParamHybrid_{size}_{replicate}.pt')

df = pd.DataFrame({"Train Loss":Train, "Test Loss": Test, "Replicate": Replicates, "Size":Size_Record, "Attempts":Attempts})

df.to_csv("Experiments/Seasonal_LV_UnknownParamHybrid_Close_Params_Results.csv",index=False)