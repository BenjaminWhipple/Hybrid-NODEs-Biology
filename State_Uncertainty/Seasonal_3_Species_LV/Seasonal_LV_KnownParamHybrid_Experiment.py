...
for size in sizes:
    for replicate in range(replicates):
        model = hybridODE(p,(3,1,[size,size])).to(device)
        ...
        torch.save(model,f'Experiments/Seasonal_LV_KnownParamHybrid/Seasonal_LV_KnownParamHybrid_{size}_{replicate}.pt')
...
df.to_csv("Experiments/Seasonal_LV_KnownParamHybrid_Results.csv",index=False)