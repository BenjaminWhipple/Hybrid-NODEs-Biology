...
# Wrong p
p = torch.tensor([1., 100., 1., 10., 100., 1., 10., 1., 10., 1., 0.1, 0.1, 1., 1.]).to(device)
...
for size in sizes:
    for replicate in range(replicates):
        ...
        torch.save(model,f'Experiments/Glycolysis_WrongParamHybrid_Models/Glycolysis_WrongParamHybrid_{size}_{replicate}.pt')

df = pd.DataFrame({"Train Loss":Train, "Test Loss": Test, "Replicate": Replicates, "Size":Size_Record, "Attempts":Attempts})

df.to_csv("Experiments/Glycolysis_WrongParamHybrid_Results.csv",index=False)

"""
# Old code

model = Known_Params_HybridODE(p,(7,7,[5,])).to(device)
