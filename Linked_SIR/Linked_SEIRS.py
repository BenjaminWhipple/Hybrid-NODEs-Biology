...
# System Parameters
beta1, beta2 = 0.4, 0.3  # Site 1 has a higher transmission rate
sigma1, sigma2 = 0.3, 0.2  # Site 2 has slower progression to infectious
...
data_size=120     # samples in dataset <- Reduced to 150 from 1000
...
# Time vector for the simulation, we want to ensure one observation per time.
t = np.linspace(0, data_size, data_size+1)  # Extended time to observe periodic behavior

# SEIRS model differential equations
def deriv(t, y):
    S1, E1, I1, R1, S2, E2, I2, R2 = y
    dS1dt = -beta1 * S1 * I1 + xi1 * R1 - mu12 * S1 + mu21 * S2
    dE1dt = beta1 * S1 * I1 - sigma1 * E1 - mu12 * E1 + mu21 * E2
    dI1dt = sigma1 * E1 - gamma1 * I1 - mu12 * I1 + mu21 * I2
    dR1dt = gamma1 * I1 - xi1 * R1 - mu12 * R1 + mu21 * R2
    dS2dt = -beta2 * S2 * I2 + xi2 * R2 - mu21 * S2 + mu12 * S1
    dE2dt = beta2 * S2 * I2 - sigma2 * E2 - mu21 * E2 + mu12 * E1
...
class SEIR_2_Site(nn.Module):
    ...
    def forward(self, t, y):
        ...
        dS1 = -self.beta1 * S1 * I1 + self.xi1 * R1 - self.mu12 * S1 + self.mu21 * S2
        dE1 = self.beta1 * S1 * I1 - self.sigma1 * E1 - self.mu12 * E1 + self.mu21 * E2
        dI1 = self.sigma1 * E1 - self.gamma1 * I1 - self.mu12 * I1 + self.mu21 * I2
        dR1 = self.gamma1 * I1 - self.xi1 * R1 - self.mu12 * R1 + self.mu21 * R2
        dS2 = -self.beta2 * S2 * I2 + self.xi2 * R2 - self.mu21 * S2 + self.mu12 * S1
        dE2 = self.beta2 * S2 * I2 - self.sigma2 * E2 - self.mu21 * E2 + self.mu12 * E1
...
t = torch.linspace(0., data_size, int(2*data_size)+1).to(device)
