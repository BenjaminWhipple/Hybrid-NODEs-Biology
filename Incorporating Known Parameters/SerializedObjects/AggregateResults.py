...
hyper_params = ["width","batch-time","learning-rate","replicate"] # Appear in file-names
...
glycolysis_known_loose_param_data = []
...
for data in glycolysis_data:
	...
	known_loose_data = [this_data["loose_err_dict"][key] for key in param_terms]
	...
	this_loose_row = these_hyper_params + known_loose_data
	this_unknown_row = these_hyper_params + unknown_data

	glycolysis_main_data.append(this_main_row)
	glycolysis_known_loose_param_data.append(this_loose_row)
...
glycolysis_known_loose_param_df = pd.DataFrame(data=glycolysis_known_loose_param_data, columns=param_columns)
...
glycolysis_known_loose_param_df.to_csv("DataFrames/Glycolysis_Known_Parameter_Estimates.csv", index = False)
...
threespecies_known_loose_param_data = []
...
for data in threespecies_data:
	...
	threespecies_known_loose_param_data.append(this_loose_row)
...
threespecies_known_loose_param_df = pd.DataFrame(data=threespecies_known_loose_param_data, columns=param_columns)
...
threespecies_known_loose_param_df.to_csv("DataFrames/ThreeSpecies_Known_Parameter_Estimates.csv", index = False)
