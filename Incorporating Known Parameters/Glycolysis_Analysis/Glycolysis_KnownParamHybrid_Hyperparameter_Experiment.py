...
p0 = torch.tensor([2.5, 100., 6., 16., 100., 1.28, 12., 1.8, 13., 4., 0.52, 0.1, 1., 4.]).to(device)
...
for replicate in range(replicates):
    for hyper_param in hyperparams:
        ...
        model = Known_Params_HybridODE(p0,(7,7,[size])).to(device)
...
df.to_csv("HyperparameterExperiments/Glycolysis_KnownParamHybrid_Results.csv",index=False)
