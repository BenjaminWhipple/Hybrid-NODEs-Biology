...
for i in range(REPLICATES):
    NODE_PREDS.append(odeint(NODE_MODELS[i], train_y0.view(1,1,7), full_t, method='rk4').view(-1,1,7))
    KNOWN_HYBRID_PREDS.append(odeint(KNOWN_HYBRID_MODELS[i], train_y0.view(1,1,7), full_t, method='rk4').view(-1,1,7))
    UNKNOWN_HYBRID_PREDS.append(odeint(UNKNOWN_HYBRID_MODELS[i], train_y0.view(1,1,7), full_t, method='rk4').view(-1,1,7))
#print(pred_y[:,0,1])

#print(data[:,0].cpu().numpy()[:,1])

fig, axs = plt.subplots(7,3,figsize=(12,19),layout='constrained',sharex="col",sharey="row")

fig.suptitle("Glycolysis Model Comparisons",fontsize=14)
...
for k in range(7):
    for l in range(3):
        axs[k,l].axvline(x=4, color='red', linestyle=':', linewidth=2)

#plt.tight_layout()
plt.savefig("GLYCOLYSIS_PREDICT.pdf")
